    ...
    <div className="flex flex-1 flex-col min-h-0 min-w-0 bg-white overflow-hidden">
          ...
          {!selected ? (
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-500">状态 {selected.status}{selected.assignedStaff ? ` · 处理人 ${selected.assignedStaff}` : ""}</p>
                <p className="mt-6 text-slate-500 text-base">对话内容将在此显示（需连接 chat-server WebSocket）</p>
              </div>
            </div>
          )}
        </section>
      ...
      <div className="shrink-0 border-t border-slate-200 bg-white relative">
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-slate-50/50 relative">
          ...
          />
          <button type="button" className="rounded-lg p-2 text-slate-500 hover:bg-slate-200 hover:text-slate-700" title="发送">
