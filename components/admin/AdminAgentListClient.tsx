  ...
  return (
    <div className="mt-6">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          <div className="py-12 text-center text-slate-500">{t("admin.agents.loading")}</div>
        ) : items.length === 0 ? (
          <div className="py-16 text-center text-slate-500">{t("admin.agents.noData")}</div>
        ) : (
          <>
              ...
              <table className="min-w-full text-[15px]">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-100/90">
                    <th className="px-4 py-3.5 text-left text-[13px] font-semibold uppercase tracking-wide text-slate-800">{t("admin.agents.regTime")}</th>
                    <th className="px-4 py-3.5 text-left text-[13px] font-semibold uppercase tracking-wide text-slate-800">User ID</th>
                    <th className="px-4 py-3.5 text-left text-[13px] font-semibold uppercase tracking-wide text-slate-800">{t("admin.agents.name")}</th>
                    <th className="px-4 py-3.5 text-left text-[13px] font-semibold uppercase tracking-wide text-slate-800">{t("admin.agents.referralCode")}</th>
                    <th className="px-4 py-3.5 text-right text-[13px] font-semibold uppercase tracking-wide text-slate-800">{t("admin.agents.l1Count")}</th>
                    <th className="px-4 py-3.5 text-left text-[13px] font-semibold tracking-wide text-slate-800">{t("admin.agents.action")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((r, idx) => (
                    <tr key={r.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <Link href={`/admin/agents/${r.id}`} className="inline-block rounded-lg border border-sky-400 bg-sky-500 px-3 py-1.5 text-sm font-bold text-white hover:bg-sky-600">{t("admin.agents.detailHierarchy")}</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
