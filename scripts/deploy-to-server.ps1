...
# 传完后需 SSH 到服务器执行：cd /root/website-new && npm install && npm run build && pm2 restart website-phase2

$sshHost = "root@68.183.227.172"
$remotePath = "/root/website-new"

$folders = @("app", "components", "config", "lib", "public", "scripts", "prisma", "docs", "services")
...
Write-Host "Done. Now SSH to server and run: cd /root/website-new && npm install && npm run build && pm2 restart website-phase2" -ForegroundColor Green
